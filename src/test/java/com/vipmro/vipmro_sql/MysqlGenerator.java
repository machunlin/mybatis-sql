    ...
    private void build() {
        ...
        AutoGenerator mpg = new AutoGenerator().setGlobalConfig(
                ...
                new DataSourceConfig()
                        ...
                        .setUrl("jdbc:mariadb://" + HOST + ":" + PORT + "/" + DATA_BASE + "?useUnicode=true&amp;characterEncoding=UTF8")
        ).setStrategy(
                // 策略配置
                new StrategyConfig()
                        // .setCapitalMode(true)// 全局大写命名
                        // .setDbColumnUnderline(true)//全局下划线命名
                        .setTablePrefix(new String[]{"sys_", "pub_", "t_", "psi_"})// 此处可以修改为您的表前缀
                        .setNaming(NamingStrategy.underline_to_camel)// 表名生成策略
                        // .setInclude(new String[] { "user" }) // 需要生成的表
                        // .setExclude(new String[]{"flyway_schema_history"}) // 排除生成的表
                        // 自定义实体父类
                        .setSuperEntityClass(BASE_PACKAGE + ".model.BaseModel")
//                        // 自定义实体，公共字段
